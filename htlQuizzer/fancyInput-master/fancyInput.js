...
;(function($){
	...
	var	letterHeight; // the clientHeight of first span in fakeDiv; setCaretHeight();

	$.fn.fancyInput = function(){
  	// second condition is false in basic page;
		if( !isIe || 'ontouchstart' in document.documentElement )
		  // 'this' is the jQuery object on which fancyInput() is called;
			init( this );
		// allow chaining by returning jQ object;
		return this;
	}
...
*/
    ...
    // ===================================
		input : function(){
  		 // save length to later check in "allEvents" callback whether text was
  		 // added;
			fancyInput.textLength = this.value.length;
    ...
    // ===================================
		// Calculate letter height for the Carot, after first letter has been
		// typed, or text pasted (only once);
		// MUCH better to check whether letterHeight is already set;
		setCaretHeight : function(inputEl){
  		console.log("IN setCaretHeight");
			var $fakeDiv = $(inputEl.nextElementSibling);
			console.log($fakeDiv.find('span').length)
			console.log(!$fakeDiv.find('span').length)
			// only if there are no spans;
			if( !$fakeDiv.find('span').length )
				return false;
			// set the height of the caret to height of first letter;
			letterHeight = $fakeDiv.find('span')[0].clientHeight;
			$fakeDiv.find('b').height(letterHeight);
		},
		
    // ===================================
		// writes a single character every time
		writer : function(charString, input, appendIndex){
  		// select all characters including <br> (which is a new line)
			...
			} else { // how could charString be false?
    ...
    // ===================================
		// insert bulk text (unlike the "writer" function which is for single
		// character only)
    ...
    // ===================================
		getCaretPosition : function(input){
			var caretPos;
			var direction = getSelectionDirection.direction || 'right';
			if( input.selectionStart || input.selectionStart == '0' )
				caretPos = direction == 'left' ? input.selectionStart : input.selectionEnd;

			return caretPos || 0;
		}
	},  // END of fancyInput object ????
  ...
  // only one call at line 45;
	...
	function init(jQObj){
  	// 'selector' is a property of the jQ object, here: ['section :input'];
		var jQselector = jQObj.selector;
    
    // the jQObj is an array of input and textarea els;
		jQObj.each(function(){
			var className = 'fancyInput';
			// &#8203; is a zero-width space char;
			var	$template = $('<div><b class="caret">&#8203;</b></div>');
				
			if( this.tagName == 'TEXTAREA' )
				className += ' textarea';
			// append a div which is the fake field;
			$(this.parentNode).append($template).addClass(className);
	
			// populate the fake field with any text that might have been on real
			// input at the time of initialization
			if( this.value )
				fancyInput.fillText(this.value, this);
			
			// transfer placeholder attr to fake div;
			if( this.placeholder ){
				$template.attr('data-placeholder', this.placeholder);
		  // add class to div if input is empty;
		  if( !this.value )
		...
		// events are all bound to 'document' but only trigger if target was among
		// the jQselector members;
		// Note that all the events are in the .fi namespace, I suppose to make it
		// easy to remove them (as in off()) without touching others;
		$(document)
		  // first remove any existing .fi namespace handlers from jQselector els;
		  .off('.fi', jQselector)
		  // input fires when input value changes;
			.on('input.fi',    jQselector, fancyInput.input)
			.on('keydown.fi',  jQselector , fancyInput.keydown)
			.on('keypress.fi', jQselector, fancyInput.keypress)
			.on('keyup.fi select.fi mouseup.fi cut.fi paste.fi blur.fi',
			                   jQselector, fancyInput.allEvents)
			.on('mousedown.fi mouseup.fi keydown.fi',
			                   jQselector, getSelectionDirection.set);
	} // END of init();
